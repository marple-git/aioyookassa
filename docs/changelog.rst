...
- Рефакторинг функции `_send_request` в `BaseAPIClient` для снижения когнитивной сложности
- Рефакторинг функции `detect` в `_MatchErrorMixin` для снижения когнитивной сложности
- Устранение дублирования кода в обработке `birthdate` в `CreatePersonalData`
- Замена дублирующегося литерала "/webhook" на константу `DEFAULT_WEBHOOK_PATH`
- Добавлены комментарии к пустым функциям в тестах для улучшения читаемости

**Технические детали:**

- Когнитивная сложность функций снижена с 18-20 до допустимых значений (<15)
- Улучшена поддерживаемость кода за счет выделения логики в отдельные методы
- Все проверки качества кода проходят (Black, MyPy, тесты)
